...
export default function VVBDashboardLayout({
    ...
    useEffect(() => {
        ...
        const savedCollapsed = localStorage.getItem("vvbSidebarCollapsed");
        if (savedCollapsed) {
    ...
    const toggleSidebarCollapse = () => {
        ...
        localStorage.setItem("vvbSidebarCollapsed", JSON.stringify(newState));
    };
    ...
    return (
        <div className="min-h-screen bg-slate-100 dark:bg-slate-900 transition-colors duration-300">
            ...
            <TooltipProvider delayDuration={0}>
                <aside
                    className={`hidden lg:block fixed top-0 left-0 h-full bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 z-50 transition-all duration-300 ${sidebarCollapsed ? "w-20" : "w-64"}`}
                >
                    {/* Logo */}
                    ...
                    <nav className={`p-2 space-y-1 overflow-y-auto h-[calc(100%-10rem)] ${sidebarCollapsed ? "px-2" : "px-4"}`}>
                        {navItems.map((item) => {
                            const isActive = pathname === item.href || (item.href !== "/vvb/dashboard" && pathname.startsWith(item.href));
                            const navLink = (
                                <Link
                                    key={item.href}
                                    href={item.href}
                                    className={`flex items-center gap-3 rounded-lg transition-all duration-200 ${sidebarCollapsed ? "justify-center px-3 py-3" : "px-3 py-2.5"} ${isActive
                    ...
                    <div className="absolute bottom-0 left-0 right-0 p-4 border-t border-slate-200 dark:border-slate-700 space-y-2">
                        <Button
                            variant="ghost"
                            className={`w-full text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50 rounded-lg ${sidebarCollapsed ? "justify-center px-0" : "justify-start gap-3"}`}
                            onClick={toggleSidebarCollapse}
                        >
                            {sidebarCollapsed ? <PanelLeft className="h-5 w-5" /> : (
                                <><PanelLeftClose className="h-5 w-5" /><span className="text-sm font-medium">Collapse</span></>
                            )}
                        </Button>
                        {sidebarCollapsed ? (
                            <Tooltip>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" className="w-full justify-center text-red-500 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20" onClick={handleLogout}>
            ...
            {/* Mobile Sidebar */}
            <aside
                className={`lg:hidden fixed top-0 left-0 h-full w-64 bg-white dark:bg-slate-800 border-r border-slate-200 dark:border-slate-700 z-50 transform transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}
            >
                {/* Logo */}
